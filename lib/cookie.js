...
const getCookie = async () => {
    ...
    try {
        ...
        console.log(`输入账号 ${config.user.mobile}`)
        ...
        while (slideNum > 0 && slideStatus == false) {
            ...
            if (jsonRes.code == 200) {
                ...
                loginRes = await page.waitForResponse(response => response.url().includes(config.juejin.loginApi) && response.status() === 200)
                try {
                    jsonRes = await loginRes.json()
                    if (jsonRes.message && jsonRes.message == 'error') {
                        console.log(`淦！登录失败[0]`)
                        console.log(jsonRes)
                    } else {
